...
def get_user():
    """ return None or a user dictionary
        if the ID is not found or if login_as was not passed
    """
    try:
...
def before_request():
    """
    use get_user to find user if any and set it as a global on flask.g.user
    """
    g.user = get_user()
